...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 NetStim1 /Users/kchen/Desktop/TA2020A/professional-workshop-3/tutorials/W4_ModelPractice/E-I-Integration/x86_64/NetStim1.mod\n");
...
static int  init_sequence (  double _lt ) {
   if ( number > 0.0 ) {
     on = 1.0 ;
     event = _lt + invl ( _threadargscomma_ interval ) ;
     end = _lt + 1e-6 ;
     }
    return 0; }
...
double invl (  double _lmean ) {
 ...
 if ( _lmean <= 0. ) {
   ...
   if ( noise  == 0.0 ) {
     ...
     _linvl = _lmean * 2.0 + normrand ( _lmean , noise * _lmean ) ;
     }
...
{
 {
   ...
   if ( start >= 0.0  && number > 0.0 ) {
     event = start + invl ( _threadargscomma_ interval ) - interval ;
     if ( event < 0.0 ) {
       event = 0.0 ;
       }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/kchen/Desktop/TA2020A/professional-workshop-3/tutorials/W4_ModelPractice/E-I-Integration/NetStim1.mod";
static const char* nmodl_file_text = 
  ": $Id: netstim.mod,v 1.5 2000/10/02 15:18:52 hines Exp $\n"
  ": comments at end\n"
  "\n"
  "NEURON	{ \n"
  "  POINT_PROCESS NetStim1\n"
  "  RANGE y\n"
  "  RANGE interval, number, start,scaleg\n"
  "  RANGE noise\n"
  "}\n"
  "\n"
  "PARAMETER {\n"
  ...
  "		event = start + invl(interval) - interval\n"
  ...
  "		invl = mean\n"
  "	}else{\n"
  "		invl = mean*2+normrand(mean,noise*mean)\n"
  "	}\n"
  "}\n"
  "\n"
